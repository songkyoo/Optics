...
public class LensGeneratorTests
{
    ...
    {
        var (_, generatedCodes) = CompileAndGetResults<LensGenerator>(
            sourceCode,
            skipGeneratedCodeCount: 0,
            additionalAssemblies: [typeof(LensOf<>).Assembly]
        );
        var generatedCode = generatedCodes.FirstOrDefault() ?? "";
    ...
    ) where T : IIncrementalGenerator, new()
    {
        ...
        var generatedCodes = generatedSources
            .Skip(skipGeneratedCodeCount)
    ...
    public void When_LensOfTypeUsed_Should_GenerateExtensionMethods()
    {
        AssertGeneratedCode(
            ...
            {
                internal static partial class LensOfExtensions
                {
    ...
    public void When_OptionalOfTypeUsed_Should_GenerateExtensionMethods()
    {
        AssertGeneratedCode(
            ...
            namespace Macaron.Optics
            {
                internal static partial class OptionalOfExtensions
                {
    ...
    public void When_MultipleTypesUsed_Should_GenerateForAllTypes()
    {
        ...
        var addressCode = generatedCodes.Single(static code => code.Contains("Address"));
        var personCode = generatedCodes.Single(static code => code.Contains("Person"));

        Assert.Multiple(() =>
        {
            Assert.That(generatedCodes, Has.Length.EqualTo(2));
            Assert.That(addressCode, Does.Contain(" Street("));
            Assert.That(addressCode, Does.Contain(" City("));
            Assert.That(addressCode, Does.Not.Contain("Person"));
            Assert.That(personCode, Does.Contain(" Name("));
            Assert.That(personCode, Does.Contain(" Age("));
            Assert.That(personCode, Does.Not.Contain("Address"));
        });
    }
